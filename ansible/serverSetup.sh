...
infoMsg "Installing Ansible to localhost"
yum -y install ansible

infoMsg "Executing the serverSetup.yml playbook."
ansible-playbook  serverSetup.yml -i inventory_files/${serverEnv}.ini  \
  --extra-vars serverEnv=${serverEnv}  \
  --extra-vars softwareSelection=${softwareSelection}  \
  --extra-vars hostsList="${hostsList}"  \
